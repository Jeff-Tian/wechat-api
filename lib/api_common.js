 ...
 * Examples:
 * ```
 * var API = require('@jeff-tian/wechat-api');
...
API.patch = function (functionName, apiUrl, override) {
  ...
  if (API.prototype[functionName] || API.prototype['_' + functionName] ) {
    if (override !== true) {
      throw new Error('@jeff-tian/wechat-api already has a prototype named ['+ functionName + '], use "true" as third param to override it or change your new function name.');
    } else {
      console.warn('@jeff-tian/wechat-api already has a prototype named ['+ functionName + '], will override the orignal one.');
    }
  }
